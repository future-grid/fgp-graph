...
Object.defineProperty(exports, "__esModule", { value: true });
const dygraphs_1 = __importDefault(__webpack_require__(/*! dygraphs */ "dygraphs"));
const moment_timezone_1 = __importDefault(__webpack_require__(/*! moment-timezone */ "./node_modules/moment-timezone/index.js"));
class Formatters {
    constructor(timezone) {
        this.timezone = timezone;
        this.legendForAllSeries = (data) => {
            if (data.x == null) {
                // This happens when there's no selection and {legend: 'always'} is set.
                return '<br>' + data.series.map(function (series) { return series.dashHTML + ' ' + series.labelHTML; }).join('<br>');
            }
            var html = moment_timezone_1.default.tz(data.x, this.timezone ? this.timezone : moment_timezone_1.default.tz.guess()).format('lll z');
        ...
        this.legendForSingleSeries = (data) => {
            if (data.x == null) {
                // This happens when there's no selection and {legend: 'always'} is set.
                return '<br>' + data.series.map(function (series) { return series.dashHTML + ' ' + series.labelHTML; }).join('<br>');
            }
        ...
        this.axisLabel = (d, granularity) => {
            // don't put it into formatters.ts becault we need to timezone later
            let momentDatetime = moment_timezone_1.default.tz(d.getTime(), this.timezone ? this.timezone : moment_timezone_1.default.tz.guess());
...
class GraphInteractions {
    constructor(callback, dateRange) {
        ...
        this.dragGetX_ = (e, context) => {
            return this.pageX(e) - context.px;
        };
        this.dragGetY_ = (e, context) => {
        ...
        this.offsetToPercentage = (g, offsetX, offsetY) => {
        ...
        this.adjustAxis = (axis, zoomInPercentage, bias) => {
            var delta = axis[1] - axis[0];
            var increment = delta * zoomInPercentage;
            var foo = [increment * bias, increment * (1 - bias)];
            return [axis[0] + foo[0], axis[1] - foo[1]];
        };
        this.zoom = (g, zoomInPercentage, xBias, yBias, direction, side, e) => {
            xBias = xBias || 0.5;
            yBias = yBias || 0.5;
            var yAxes = g.axes_;
            var newYAxes = [];
...
class Synchronizer {
    constructor(graphs) {
        this.synchronize = () => {
            ...
            if (this.args[0] instanceof dygraphs_1.default) {
                // Arguments are Dygraph objects.
                for (var i = 0; i < this.args.length; i++) {
                    if (this.args[i] instanceof dygraphs_1.default) {
...
Object.defineProperty(exports, "__esModule", { value: true });
const DomElements_1 = __webpack_require__(/*! ./widgets/DomElements */ "./src/widgets/DomElements.ts");
class FgpGraph {
    constructor(dom, viewConfigs) {
        this.fieldPattern = new RegExp(/data[.]{1}[a-zA-Z0-9]+/g);
        this.childrenGraphs = [];
        // store locally
        this.rangeBarData = [];
        this.serialnumber = -1;
        this.datewindowHandler = (datewindow) => {
         ...
         */
        this.initGraph = () => {
            this.operator = new DomElements_1.GraphOperator(this.graph, this.rangeBarGraph, this.graphContainer, this.body, this.intervalsDropdown, this.header, this.datewindowHandler);
            ...
            if (this.viewConfigs) {
                ...
                const viewsDropdonwOptions = new DomElements_1.DropdownButton(this.viewsDropdown, [...dropdownOpts]);
                viewsDropdonwOptions.render();
                // add callback handler
                this.viewsDropdown.onchange = (e) => {
                    const choosedView = e.target.value;
                    // change view
        ...
        this.updateDatewinow = (datewindow) => {
            // update graph 
            if (this.graph) {
                const range = this.graph.xAxisRange();
        ...
        // create doms
        let containerAttrs = [{ key: 'class', value: 'fgp-graph-container noselect' }];
        ...
        let bodyAttrs = [{ key: 'class', value: 'fgp-graph-body' }];
...
const dygraphs_1 = __importDefault(__webpack_require__(/*! dygraphs */ "dygraphs"));
const moment_timezone_1 = __importDefault(__webpack_require__(/*! moment-timezone */ "./node_modules/moment-timezone/index.js"));
const synchronizer_1 = __webpack_require__(/*! ../extras/synchronizer */ "./src/extras/synchronizer.ts");
const interactions_1 = __webpack_require__(/*! ../extras/interactions */ "./src/extras/interactions.ts");
const formatters_1 = __webpack_require__(/*! ../extras/formatters */ "./src/extras/formatters.ts");
class DropdownButton {
    constructor(select, buttons) {
        /**
         * generate options
         *
...
class DropdownMenu {
    constructor(dropdownArea, opts, callback) {
        this.render = () => {
        ...
        <select>
            ...
            this.opts.forEach(opt => {
                ...
                checkbox.addEventListener("click", (e) => {
                    //
                    let series = e.target.getAttribute("data-value");
...
class DomElementOperator {
}
DomElementOperator.createElement = (type, attrs) => {
...
class GraphOperator {
    constructor(mainGraph, rangeGraph, graphContainer, graphBody, intervalsDropdown, header, datewindowCallback) {
        this.defaultGraphRanges = [
            { name: "3 days", value: (1000 * 60 * 60 * 24 * 3), show: true },
            { name: "7 days", value: 604800000, show: true },
            { name: "1 month", value: 2592000000, show: false }
        ];
        this.createElement = (type, attrs) => {
            let dom = document.createElement(type);
            // put attributes on element
            attrs.forEach(attr => {
                dom.setAttribute(attr.key, attr.value);
         ...
         */
        this.updateCollectionLabels = (header, entities, choosedCollection, collections) => {
            // 
            let labels = header.getElementsByClassName('fgp-interval-labels'); // should only have one.
            let firstLabelArea = null;
        ...
        this.updateSeriesDropdown = (header, series, graph) => {
            ...
            let select = this.createElement("div", []);
            ...
            new DropdownMenu(select, opts, (series, checked) => {
                let visibility = graph.getOption('visibility');
                let labels = graph.getLabels();
        ...
        this.init = (view, readyCallback, interactionCallback) => {
            this.currentView = view;
            let formatters = new formatters_1.Formatters(this.currentView.timezone ? this.currentView.timezone : moment_timezone_1.default.tz.guess());
            let entities = [];
            let bottomAttrs = [{ key: 'class', value: 'fgp-graph-bottom' }];
            let bottom = null;
            this.currentView.graphConfig.entities.forEach(entity => {
                entities.push(entity.id);
            });
            // find fields from configuration
            let timewindowEnd = moment_timezone_1.default.tz(this.currentView.timezone ? this.currentView.timezone : moment_timezone_1.default.tz.guess()).add(1, 'days').startOf('day').valueOf();
            let timewindowStart = moment_timezone_1.default.tz(this.currentView.timezone ? this.currentView.timezone : moment_timezone_1.default.tz.guess()).subtract(7, 'days').startOf('day').valueOf(); // default 7 days
            const ranges = this.currentView.ranges;
            if (ranges.length > 0) {
                // get first "show" == true
                ...
                if (!selected) {
                    ...
                    timewindowStart = moment_timezone_1.default.tz(this.currentView.timezone ? this.currentView.timezone : moment_timezone_1.default.tz.guess()).add(1, 'days').startOf('day').valueOf() - ranges[0].value;
                }
                else {
                    timewindowStart = moment_timezone_1.default.tz(this.currentView.timezone ? this.currentView.timezone : moment_timezone_1.default.tz.guess()).add(1, 'days').startOf('day').valueOf() - selected.value;
                }
            }
            ...
            let choosedCollection = null;
            const intervalsDropdonwOptions = new DropdownButton(this.intervalsDropdown, [...dropdownOpts]);
            intervalsDropdonwOptions.render();
            this.intervalsDropdown.onchange = (e) => {
                const intervalDropdown = e.currentTarget;
                graphRangesConfig.forEach(config => {
            ...
            this.currentView.dataService.fetchFirstNLast(entities, this.currentView.graphConfig.rangeCollection.name, Array.from(new Set(fieldsForCollection))).then(resp => {
                ...
                let first = { timestamp: moment_timezone_1.default.tz(this.currentView.timezone ? this.currentView.timezone : moment_timezone_1.default.tz.guess()).valueOf() };
                ...
                if (!choosedCollection && firstRanges) {
                    // cal with width
                    const width = this.graphContainer.offsetWidth;
                    //
                    const pointsCanBeShown = Math.round(width * .9);
                    this.currentView.graphConfig.collections.forEach(collection => {
                ...
                const datewindowChangeFunc = (e, yAxisRange) => {
                    let datewindow = [];
                    if (this.ragnebarGraph) {
                    ...
                    if (datewindow[0] == currentDatewindowOnMouseDown[0] && datewindow[1] == currentDatewindowOnMouseDown[1]) {
                        ...
                        let collection = { label: "", name: "", series: [], interval: 0, initScales: { left: { min: 0, max: 0 }, right: { min: 0, max: 0 } } };
                ...
                let dateLabelLeftAttrs = [{ key: 'class', value: 'fgp-graph-range-bar-date-label-left' }];
                let startLabelLeft = DomElementOperator.createElement('label', dateLabelLeftAttrs);
                let dateLabelRightAttrs = [{ key: 'class', value: 'fgp-graph-range-bar-date-label-right' }];
                let endLabelRight = DomElementOperator.createElement('label', dateLabelRightAttrs);
                // create a interaction model instance
                let interactionModel = new interactions_1.GraphInteractions(callbackFuncForInteractions, [first.timestamp, last.timestamp]);
                let currentSelection = "";
                this.mainGraph = new dygraphs_1.default(this.graphBody, initialData, {
                    ...
                    drawCallback: (dygraph, is_initial) => {
                        ...
                        if (this.currentView.graphConfig.features.rangeBar && this.currentView.graphConfig.rangeCollection) {
                            startLabelLeft.innerHTML = moment_timezone_1.default.tz(xAxisRange[0], this.currentView.timezone ? this.currentView.timezone : moment_timezone_1.default.tz.guess()).format('lll z');
                            endLabelRight.innerHTML = moment_timezone_1.default.tz(xAxisRange[1], this.currentView.timezone ? this.currentView.timezone : moment_timezone_1.default.tz.guess()).format('lll z');
                        }
                        // update datewindow
                ...
                if (this.currentView.graphConfig.features.rangeBar && this.currentView.graphConfig.rangeCollection) {
                    ...
                    let dateLabels = DomElementOperator.createElement('div', [{ key: 'style', value: 'height:22px;' }]);
                    ...
                    let rangeBarAttrs = [{ key: 'class', value: 'fgp-graph-rangebar' }];
                    let rangeBar = DomElementOperator.createElement('div', rangeBarAttrs);
                    bottom.appendChild(rangeBar);
                    this.graphContainer.appendChild(bottom);
                    //create range-bar graph
                    this.ragnebarGraph = new dygraphs_1.default(rangeBar, [
                        firstData,
                        lastData // last
                    ], {
                        xAxisHeight: 0,
                    ...
                    // check 
                    let sync = new synchronizer_1.Synchronizer([this.ragnebarGraph, this.mainGraph]);
                    sync.synchronize();
        ...
        this.refresh = () => {
            const xAxisRange = this.mainGraph.xAxisRange();
            let datewindow = [];
            if (xAxisRange) {
        ...
        this.update = (first, last) => {
            ...
            let formatters = new formatters_1.Formatters(view.timezone ? view.timezone : moment_timezone_1.default.tz.guess());
            let mainGraphColors = [];
            ...
            let prepareGraphData = (data, entities, collection) => {
                // update main graph
                let graphData = [];
                let finalData = [];
                let _dates = [];
            ...
            view.dataService.fetchdata(mainEntities, graphCollection.name, { start: start, end: end }, Array.from(new Set(fieldsForMainGraph))).then(resp => {
                ...
                let yScale = { valueRange: [] };
                let y2Scale = { valueRange: [] };
                // get init scale
            ...
            if (view.graphConfig.features.rangeBar) {
                ...
                const rangeEntities = [view.graphConfig.rangeEntity.id];
                // get fields for main graph
                ...
                view.dataService.fetchdata(rangeEntities, rangeCollection.name, { start: start, end: end }, Array.from(new Set(fieldsForRangebarGraph))).then(resp => {
                    ...
                    let preData = rangebarGraph.file_;
...
module.exports = require("Dygraph");
